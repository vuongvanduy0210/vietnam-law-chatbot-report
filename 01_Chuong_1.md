...
FastAPI là một web framework hiện đại, hiệu suất cao được viết bằng Python (yêu cầu phiên bản 3.7+), được thiết kế chuyên biệt để xây dựng các API một cách nhanh chóng và hiệu quả. Framework này được xây dựng trên nền tảng của Starlette (cho các phần web) và Pydantic (cho việc xác thực dữ liệu).

**Ưu điểm:**

- Hiệu năng vượt trội: Nhờ xây dựng trên Starlette và hỗ trợ asyncio native, FastAPI có hiệu năng rất cao, lý tưởng cho các endpoint cần SSE streaming trả lời realtime.
- Tốc độ phát triển cực nhanh: FastAPI giúp giảm từ 200% đến 300% thời gian phát triển code so với Flask truyền thống.
- Tự động tạo tài liệu tương tác: FastAPI tự động sinh tài liệu API theo chuẩn OpenAPI (Swagger) và ReDoc từ type hints mà không cần viết tay.
- Xác thực dữ liệu mạnh mẽ: Với Pydantic, việc xác thực, tuần tự hoá dữ liệu trở nên cực kỳ đơn giản và rõ ràng.
- Hỗ trợ lập trình bất đồng bộ: FastAPI được thiết kế với tư tưởng "async first", tối ưu cho các tác vụ I/O-bound như gọi LLM API và truy vấn CSDL.

**Nhược điểm:**

- Yêu cầu kiến thức về type hints và async/await — rào cản với người mới.
- Cộng đồng nhỏ hơn Django/Flask, ít plugin sẵn có hơn.

Trong hệ thống, triển khai hai service FastAPI riêng biệt: `main-service` (port 8000) xử lý xác thực và quản lý người dùng; `rag-service` (port 8001) chứa toàn bộ pipeline Agentic RAG.

### 1.5.4. Next.js 16 — Web Admin Dashboard

Next.js là framework React do Vercel phát triển, được sử dụng rộng rãi cho các ứng dụng web fullstack. Phiên bản sử dụng trong đề tài là **Next.js 16** (2025) với React 19, App Router, Server Components và Turbopack.

**Ưu điểm:**

- Hỗ trợ Server Components và App Router — tăng hiệu năng, giảm JavaScript bundle gửi về client.
- Tích hợp sẵn SSR (Server-Side Rendering) và SSG (Static Site Generation).
- Turbopack giúp hot reload nhanh hơn đáng kể so với Webpack truyền thống.
- TypeScript first — đảm bảo type safety xuyên suốt codebase.

**Nhược điểm:**

- App Router còn tương đối mới, một số thư viện trong ecosystem chưa tương thích hoàn toàn.
- Cấu hình phức tạp hơn Create React App với dự án nhỏ.

Trong đề tài, Next.js được dùng cho **Admin Dashboard** — cho phép quản trị viên tải lên văn bản pháp luật mới, giám sát tiến trình nạp dữ liệu từng bước qua WebSocket và xem thống kê CSDL theo thời gian thực.

### 1.5.5. Kotlin Multiplatform + Compose Multiplatform — Ứng dụng di động

**Kotlin Multiplatform (KMP)** là công nghệ của JetBrains cho phép chia sẻ code Kotlin giữa Android, iOS, Desktop và Web trong cùng một dự án. **Compose Multiplatform** là thư viện UI declarative port từ Jetpack Compose của Android, cho phép viết giao diện một lần bằng Kotlin và chạy trên cả Android lẫn iOS.

**Ưu điểm:**

- Chia sẻ khoảng 95% code business logic và UI giữa Android/iOS — tối ưu nguồn lực khi chỉ có một developer.
- Hiệu năng cao hơn Flutter: compile sang native bytecode thực sự trên cả hai nền tảng.
- Hệ sinh thái Kotlin đầy đủ: Ktor (HTTP/WebSocket), Koin (Dependency Injection), kotlinx-serialization (JSON).
- Tận dụng toàn bộ kiến thức Android/Jetpack Compose — không cần học ngôn ngữ mới.

**Nhược điểm:**

- Ecosystem iOS còn trẻ — một số thư viện chưa ổn định bằng phiên bản Android.
- Tài liệu KMP ít hơn Flutter và React Native, cộng đồng còn nhỏ.
- Các phần platform-specific (camera, Bluetooth) vẫn cần viết riêng cho từng nền tảng.

Phiên bản sử dụng trong đề tài: **Kotlin 2.3.0**, **Compose Multiplatform 1.10.0**, hỗ trợ Android (SDK 24–36) và iOS (arm64 + simulator).

### 1.5.6. Hệ thống cơ sở dữ liệu đa mô hình (Polyglot Persistence)

Hệ thống Vietnam Law Chatbot áp dụng chiến lược **Polyglot Persistence** — sử dụng nhiều loại CSDL trong cùng một hệ thống, mỗi CSDL được chọn dựa trên đặc điểm của dữ liệu cần lưu trữ.

#### 1.5.6.1. PostgreSQL — Dữ liệu giao dịch

PostgreSQL (thường gọi là "Postgres") là hệ quản trị CSDL quan hệ-đối tượng mã nguồn mở với hơn 35 năm phát triển, nổi tiếng về độ tin cậy và tuân thủ chuẩn ACID.

**Ưu điểm:**

- Tuân thủ nghiêm ngặt ACID — đảm bảo toàn vẹn dữ liệu giao dịch người dùng.
- Hỗ trợ MVCC (Multi-Version Concurrency Control) — nhiều tiến trình đọc/ghi song song không khóa lẫn nhau.
- Hỗ trợ JSON/JSONB với đánh chỉ mục — cạnh tranh được với NoSQL.
- Mã nguồn mở, không phí bản quyền, cộng đồng phát triển vững mạnh.

**Nhược điểm:**

- Cấu hình và tối ưu phức tạp hơn MySQL với dữ liệu rất lớn.
- Schema linh hoạt kém hơn NoSQL khi dữ liệu có cấu trúc thay đổi thường xuyên.

Trong đề tài, PostgreSQL lưu **dữ liệu giao dịch** gồm 5 bảng: `users`, `conversations`, `messages`, `document_tasks`, `refresh_tokens`. ORM sử dụng là SQLAlchemy 2.0 với async qua `asyncpg`.

#### 1.5.6.2. MongoDB — Kho văn bản pháp luật

MongoDB là CSDL NoSQL theo mô hình tài liệu (document-oriented), lưu dữ liệu dưới dạng BSON (Binary JSON).

**Ưu điểm:**

- Schema linh hoạt — phù hợp với văn bản pháp luật có cấu trúc metadata đa dạng (số điều, chủ đề, từ khoá thay đổi giữa các loại văn bản).
- Hiệu năng đọc/ghi cao với document có kích thước lớn như toàn văn điều luật.
- Hỗ trợ sharding và replica set cho quy mô dữ liệu lớn.

**Nhược điểm:**

- Không đảm bảo ACID ở cấp độ multi-document transaction như PostgreSQL.
- Tốn bộ nhớ hơn vì lưu tên trường trong mỗi document.

Trong đề tài, MongoDB lưu **toàn văn điều luật** trong collection `VietnamLawDB.articles` — theo dữ liệu thực tế, chứa **528.620 documents** trải dài từ 1945 đến 2026. Driver async sử dụng là `motor`.

#### 1.5.6.3. ChromaDB — Kho vector embedding

ChromaDB là vector database mã nguồn mở chuyên dụng do Chroma Inc. phát triển từ năm 2023, được tối ưu hoá cho bài toán tìm kiếm ngữ nghĩa (semantic search).

**Ưu điểm:**

- Python-native, tích hợp thuận tiện với FastAPI và LangChain/LangGraph.
- Hỗ trợ sẵn HNSW index và cosine distance — phù hợp với vector embedding ngôn ngữ.
- Triển khai cục bộ qua Docker, không phát sinh chi phí.
- API đơn giản, dễ tích hợp trong pipeline RAG.

**Nhược điểm:**

- Chưa hỗ trợ tốt cho quy mô doanh nghiệp lớn so với Milvus hay Weaviate.
- Thiếu các tính năng quản lý và giám sát nâng cao.

Trong đề tài, collection `vietnamese_law` chứa **690.360 chunks** (768 chiều), mỗi chunk mang metadata: `law_id`, `article_id`, `year`, `topics`, `keywords`. Cấu hình HNSW: `ef_construction=100, ef_search=100, max_neighbors=16`, thời gian truy vấn Top-60 dưới 200ms.

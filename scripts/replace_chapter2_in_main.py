...
CHAPTER2 = ROOT / "Bao_Cao" / "02_Chuong_2_ban_dinh_dang_theo_mau.docx"
BACKUP = ROOT / "Vương Văn Duy_Báo cáo_backup_truoc_thay_chuong2.docx"
...
# Source chapter style IDs differ from the main report style IDs even though the
# Vietnamese style names are visually the same.
STYLE_ID_REMAP = {
    "Tênhìnhvẽ": "Tnhnhv",
    "Tênbảng": "Tnbng",
}
...
def find_chapter_bounds(body):
    ...
    for i, el in enumerate(body):
        ...
        if style == "Heading1" and text.startswith("CHƯƠNG 2"):
            start = i
        elif start is not None and style == "Heading1" and text.startswith("CHƯƠNG 3"):
            end = i
            break
    if start is None or end is None or end <= start:
        raise RuntimeError("Không xác định được ranh giới Chương 2 trong file chính")
    return start, end
...
def ensure_unique_media_name(media_dir: Path, original_name: str) -> str:
    ...
    idx = 1
    while True:
        candidate = f"chapter2_{stem}_{idx}{suffix}"
...
def main():
    ...
    if not CHAPTER2.exists():
        raise FileNotFoundError(CHAPTER2)
    ...
    with TemporaryDirectory() as tmp:
        tmp = Path(tmp)
        main_dir = tmp / "main"
        src_dir = tmp / "chapter2"
        unzip_docx(MAIN, main_dir)
        unzip_docx(CHAPTER2, src_dir)
        ...
        # Prepare source chapter body elements, excluding the standalone sectPr.
        source_elements = [
            deepcopy(el)
            for el in src_body
        ...
        # Remove old Chapter 2 body content.
        for el in list(main_body)[start:end]:
            main_body.remove(el)

        # Insert new Chapter 2 at the original position.

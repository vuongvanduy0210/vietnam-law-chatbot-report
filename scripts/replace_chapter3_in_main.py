...
CHAPTER3 = ROOT / "Bao_Cao" / "03_Chuong_3.docx"
BACKUP = ROOT / "Vương Văn Duy_Báo cáo_backup_truoc_thay_chuong3.docx"
...
def find_chapter_bounds(body):
    ...
    for i, el in enumerate(body):
        ...
        if style == "Heading1" and text.startswith("CHƯƠNG 3"):
            start = i
        elif start is not None and style == "Heading1" and text.startswith("CHƯƠNG 4"):
            end = i
            break

    if start is None:
        raise RuntimeError("Không xác định được điểm bắt đầu Chương 3 trong file chính")
    ...
    if end <= start:
        raise RuntimeError("Không xác định được ranh giới Chương 3 trong file chính")
    return start, end
...
def ensure_unique_media_name(media_dir: Path, original_name: str) -> str:
    ...
    idx = 1
    while True:
        candidate = f"chapter3_{stem}_{idx}{suffix}"
...
def main():
    ...
    if not CHAPTER3.exists():
        raise FileNotFoundError(CHAPTER3)
    ...
    with TemporaryDirectory() as tmp:
        tmp = Path(tmp)
        main_dir = tmp / "main"
        src_dir = tmp / "chapter3"
        unzip_docx(MAIN, main_dir)
        unzip_docx(CHAPTER3, src_dir)
